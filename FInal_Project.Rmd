...
# table_final <- as.data.frame(print(table1, showAllLevels = TRUE, formatOptions = list(big.mark = ',')))
# 
# table_final <- data.frame(table1)
# 
# library(kableExtra)
...
# tab <- kable(x = table_final, caption = 'Summary Table',
#              format = 'latex',booktabs = T,
#              col.names = c('Variable','Overall','COVID-19 Positive','COVID-19 Negative'),
#              align = c('l','r','r','r'),escape = T) %>%
...
colnames(df_p_icu) <- c('var1','disease_status','Percent')
df_p_icu$Percent <- df_p_icu$Percent * 100



final_df_sex <- dplyr::inner_join(df_n_sex, df_p_sex, by = c('var1','disease_status'))

final_df_icu <- dplyr::inner_join(df_n_icu, df_p_icu, by = c('var1','disease_status'))
...
pos_summary <- as.data.frame(apply(pos_vars %>% 
                                     dplyr::select(age, ferritin.ng.ml., lactate.mmol.l.), 2, FUN = function(x) {contSummary(x, normal = TRUE)}))
neg_summary <- as.data.frame(apply(neg_vars %>% 
                                     dplyr::select(age, ferritin.ng.ml., lactate.mmol.l.), 2, FUN = function(x) {contSummary(x, normal = TRUE)}))
...
final_table <- cat_table %>%
  dplyr::rename('Variables' = 'var1', 'Covid.Positive' = 'disease state: COVID-19', 'Covid.Negative' = 'disease state: non-COVID-19')
